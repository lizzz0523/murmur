...
macOS 上的本地语音输入工具：按住快捷键说话，边录边识别，松开后自动润色并粘贴到当前光标处。所有推理都在本机完成，音频不会离开设备。

## 功能特性

- **按住说话**：按住右侧 `Ctrl`（或 `Fn`）录音；录制过程中实时识别，松开后润色并输入。
- **悬浮胶囊**：屏幕底部居中的置顶胶囊实时显示状态（Ready / 录音波形 / Thinking...）。
- **流式识别**：录音过程中实时 VAD 分段并逐段识别，识别与说话重叠；松手后立即润色。
...
录音中（流式，与说话重叠）：
  重采样到 16kHz → 高通滤波
    → Silero VAD 实时分段 → 分段 GTCRN 降噪 → 响度归一化 → 峰值限制
    → Qwen3-ASR 逐段识别（结果累积，不显示）

松手：
  长文本按窗口分块 → Qwen3-4B 全局润色
    → 一次性输入到当前焦点
```
...
src/
├── app.rs         应用状态机与界面绘制
├── audio.rs       降混、高通滤波、归一化、峰值限制
├── hotkey.rs      全局快捷键监听
├── hub.rs         模型下载与进度（resolve_model）
├── main.rs        程序入口、窗口配置与图标
├── recognizer.rs  流式重采样、VAD 分段、降噪与逐段 ASR
├── recorder.rs    音频录制、设备枚举与切换
├── refiner.rs     长文本分窗的 Qwen3 润色（llama.cpp 推理）
└── tray.rs        菜单栏图标与菜单
assets/            应用图标与托盘模板图标
```
